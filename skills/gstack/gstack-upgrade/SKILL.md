...
[ -z "$_AUTO" ] && _AUTO=$(skills/gstack/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
```bash
skills/gstack/bin/gstack-config set auto_upgrade true
```
...
**If "Never ask again":**
```bash
skills/gstack/bin/gstack-config set update_check false
```
Tell user: "Update checks disabled. Run `skills/gstack/bin/gstack-config set update_check true` to re-enable."
Continue with the current skill.

### Step 2: Detect install type

```bash
if [ -d "$HOME/skills/gstack/.git" ]; then
  INSTALL_TYPE="global-git"
  INSTALL_DIR="$HOME/.claude/skills/gstack"
elif [ -d "skills/gstack/.git" ]; then
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `skills/gstack/` when you're ready."
...
```bash
skills/gstack/bin/gstack-update-check --force 2>/dev/null || \
skills/gstack/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `skills/gstack/` when you're ready."
